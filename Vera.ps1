...
[CurrentVersion]1.4
[Nyheter]"* Nu går det att kontrollera vilket mode veran är i, samt snabbt byta till ett nytt mode."
...
#>
    ...
    begin{
    try{
    $version = $(invoke-webrequest -uri "http://www.automatiserar.se/wp-content/uploads/2014/12/Automatiserar.txt").content.split("`n") | Where-Object {$_ -match "\[currentversion\]" -or $_ -match "\[Nyheter\]" -or $_ -match "\[OLDVersion\]"}
    } 
    catch{
    Write-Error "Fel"
    }
...
# Nya i version 1.2
  ...
  switch ($DeviceType)
  {
      ...
      'HaDevice1'          {$PSVeraDevice | Add-Member -MemberType NoteProperty -Name "EnhetsTyp" -Value $SensorNameHaDevice1           ; $FixedDevice = $false;break}
  ...
  Write-Verbose "[Device-ToObject] Översätter enheten till ett objekt"
